...
fn string_field_resolved_from_toml() {
    with_temp_xdg(|_| {
        let spec = CredentialsSpec { fields: vec![field("token", "string", true)] };
...
fn file_field_reads_file_content() {
    with_temp_xdg(|xdg| {
        ...
        fs::write(creds_dir.join("default_pubkey"), "-----BEGIN PGP PUBLIC KEY-----\n...").unwrap();
        let spec = CredentialsSpec { fields: vec![field("default_pubkey", "file", false)] };
        let toml = HashMap::new();
        let json = build_credentials_json(Some(&spec), "testplugin", &toml, Lang::En).unwrap();
        let v: serde_json::Value = serde_json::from_str(&json).unwrap();
        assert!(v["default_pubkey"].as_str().unwrap().contains("BEGIN PGP PUBLIC KEY"));
    });
}
...
fn file_field_missing_injects_empty_string() {
    with_temp_xdg(|_| {
        let spec = CredentialsSpec { fields: vec![field("nonexistent", "file", false)] };
        let toml = HashMap::new();
        let json = build_credentials_json(Some(&spec), "testplugin", &toml, Lang::En).unwrap();
        let v: serde_json::Value = serde_json::from_str(&json).unwrap();
...
fn credentials_dir_auto_injected_even_without_spec() {
    with_temp_xdg(|_| {
        ...
        let v: serde_json::Value = serde_json::from_str(&json).unwrap();
        assert!(v["__credentials_dir"].as_str().unwrap().ends_with("credentials.d/simpleplugin"));
    });
}
...
fn directory_field_returns_subdir_path() {
    with_temp_xdg(|_| {
        let spec = CredentialsSpec { fields: vec![field("keyring", "directory", false)] };
        let json = build_credentials_json(Some(&spec), "testplugin", &HashMap::new(), Lang::En).unwrap();
        let v: serde_json::Value = serde_json::from_str(&json).unwrap();
        assert!(v["keyring"].as_str().unwrap().ends_with("credentials.d/testplugin/keyring"));
    });
}
