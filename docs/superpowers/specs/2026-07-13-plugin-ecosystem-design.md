...
| 双语适配 (i18n) | 所有用户可见消息经 `src/i18n.rs` 的 `pub fn` 走 Lang 双语；插件错误对象的 message 透传不翻译 |
...
- [src/i18n.rs](file:///workspace/src/i18n.rs) 新增插件相关双语消息函数（见 §13）。所有 CLI 子命令输出、运行时插件错误、manifest 校验失败、SHA256 校验失败、兼容性 gate 拒绝、`list`/`autoremove` 输出均经 i18n。
...
**i18n 双语断言**（新增一组，对照 §12.2 函数清单）

- §12.2 列出的每个新增 `pub fn` 在 `Lang::En` 与 `Lang::Zh` 下都返回非空字符串
- 关键函数在两语言下返回**不同**字符串（如 `plugin_sha256_mismatch` 的 en/zh 输出不相等）
...
11. i18n 函数（`src/i18n.rs`）——§12.2 清单全量实现，先于或并行于 CLI/运行时消息调用点
12. 测试 fixture 与集成测试（含多级命令、CLI 子命令、i18n 双语断言用例）
13. server.js 扩展（新增 `/plugins/...` 路由）
14. 发布脚本 `scripts/publish-plugin.sh`
15. 文档（README + dyyl-api-reference.md 增"插件系统"章节，含 UB 风险警告、多级命令说明、CLI 子命令用法、i18n 约定）
...
所有用户可见消息必须经 `src/i18n.rs` 的 `pub fn <name>(lang: Lang, ...) -> &'static str | String` 走双语，与现有 `cli_version_banner` / `unknown_command` / `mcm_no_host_provider` 等函数一致。**不允许**在 CLI 或运行时代码里硬编码英文字符串。

**消息归属边界：**

| 消息来源 | 翻译责任 | 经 i18n.rs？ |
|---|---|---|
| dyyl 自己生成的消息（manifest 校验、SHA256、网络、ABI、路由、CLI 输出、`list`/`autoremove` 摘要等） | dyyl | 是，必须 |
| 插件返回的 error 对象 `message` 字段 | 插件作者 | 否，dyyl 透传原文 |
| 插件 `list_commands` 输出的 `brief` 字段 | 插件作者 | 否，dyyl 透传原文 |
| 插件 `get_command_help` 输出 | 插件作者 | 否，dyyl 透传原文 |
| 错误对象 `code` 枚举 | 机器可读，不翻译 | 否 |

### 12.2 新增 i18n 函数清单

在 `src/i18n.rs` 新增以下函数（命名遵循现有 `pub fn <context>_<action>(lang, ...) -> ...` 风格）：

**CLI 子命令输出**

| 函数 | En | Zh |
|---|---|---|
| `plugin_install_success(lang, name, ver)` | `installed {name} {ver}` | `已安装 {name} {ver}` |
| `plugin_already_installed(lang, name, ver)` | `{name} {ver} already installed` | `{name} {ver} 已安装` |
| `plugin_updated(lang, name, old, new)` | `updated {name} {old} -> {new}` | `已升级 {name} {old} -> {new}` |
| `plugin_already_latest(lang, name, ver)` | `{name} already latest ({ver})` | `{name} 已是最新 ({ver})` |
| `plugin_removed(lang, name)` | `removed {name}` | `已卸载 {name}` |
| `plugin_not_installed(lang, name)` | `error: {name} not installed` | `错误: {name} 未安装` |
| `plugin_install_failed(lang, name, reason)` | `error: failed to install {name}: {reason}` | `错误: 安装 {name} 失败: {reason}` |
| `plugin_update_failed(lang, name, reason)` | `error: failed to update {name}: {reason}` | `错误: 升级 {name} 失败: {reason}` |
| `plugin_remove_failed(lang, name, reason)` | `error: failed to remove {name}: {reason}` | `错误: 卸载 {name} 失败: {reason}` |
| `plugin_update_all_summary(lang, updated, latest, failed)` | `updated {updated}, already-latest {latest}, failed {failed}` | `已升级 {updated}, 已最新 {latest}, 失败 {failed}` |
| `plugin_autoremove_summary(lang, count)` | `autoremoved {count} plugins` | `自动清理了 {count} 个插件` |
| `plugin_autoremove_removed(lang, name, days_ago)` | `removed {name} (last used {days_ago} days ago)` | `已卸载 {name} (上次使用 {days_ago} 天前)` |
| `plugin_list_header(lang)` | `NAME VERSION LAST_USED INSTALLED` | `名称 版本 上次使用 安装时间` |
| `plugin_list_empty(lang)` | `no plugins installed` | `未安装任何插件` |
| `plugin_list_never_used(lang)` | `-` | `-` |

**`list` 表头与字段名翻译**：表格表头走 i18n（`名称/版本/上次使用/安装时间`）；插件名与版本号不翻译；`last_used_at` 缺失显示 `-`。

**运行时插件错误**

| 函数 | En | Zh |
|---|---|---|
| `plugin_fetch_manifest_failed(lang, name, reason)` | `failed to fetch manifest for '{name}': {reason}` | `获取 '{name}' 清单失败: {reason}` |
| `plugin_manifest_not_found(lang, name)` | `plugin '{name}' not found on l.dyyapp.com` | `l.dyyapp.com 上未找到插件 '{name}'` |
| `plugin_abi_mismatch(lang, name, expected, actual)` | `plugin '{name}' ABI version mismatch: expected {expected}, got {actual}` | `插件 '{name}' ABI 版本不匹配: 期望 {expected}, 实际 {actual}` |
| `plugin_dyyl_min_unmet(lang, name, required, current)` | `plugin '{name}' requires dyyl >= {required}, current is {current}` | `插件 '{name}' 需要 dyyl >= {required}, 当前 {current}` |
| `plugin_platform_unavailable(lang, name, platform, available)` | `plugin '{name}' has no build for {platform}; available: {available}` | `插件 '{name}' 无 {platform} 构建; 可用: {available}` |
| `plugin_sha256_mismatch(lang, name)` | `plugin '{name}' SHA256 checksum mismatch` | `插件 '{name}' SHA256 校验和不匹配` |
| `plugin_download_failed(lang, name, reason)` | `failed to download plugin '{name}': {reason}` | `下载插件 '{name}' 失败: {reason}` |
| `plugin_dlopen_failed(lang, name, reason)` | `failed to load plugin '{name}': {reason}` | `加载插件 '{name}' 失败: {reason}` |
| `plugin_symbol_missing(lang, name, symbol)` | `plugin '{name}' missing required symbol '{symbol}'` | `插件 '{name}' 缺少必需符号 '{symbol}'` |
| `plugin_init_failed(lang, name)` | `plugin '{name}' init() returned NULL` | `插件 '{name}' init() 返回 NULL` |
| `plugin_on_load_failed(lang, name, code)` | `plugin '{name}' on_load() failed with code {code}` | `插件 '{name}' on_load() 失败，码 {code}` |
| `plugin_unknown_subcommand(lang, name, sub)` | `plugin '{name}' has no command '{sub}'` | `插件 '{name}' 无命令 '{sub}'` |
| `plugin_arity_mismatch(lang, name, sub, expected, actual)` | `plugin command '{name}.{sub}' expects {expected} args, got {actual}` | `插件命令 '{name}.{sub}' 期望 {expected} 个参数, 实际 {actual}` |
| `plugin_command_failed(lang, name, sub, code)` | `plugin command '{name}.{sub}' failed: {code}` | `插件命令 '{name}.{sub}' 失败: {code}` |
| `plugin_panic_warning(lang, name)` | `warning: plugin '{name}' panicked; behavior is undefined (panic_mode not isolated)` | `警告: 插件 '{name}' panic; 行为未定义 (panic_mode 未隔离)` |

**CLI 用法与版本横幅**

| 函数 | En | Zh |
|---|---|---|
| `cli_plugin_usage(lang)` | `Usage: dyyl install <name> | update [name] | remove <name> | autoremove | list` | `用法: dyyl install <名称> | update [名称] | remove <名称> | autoremove | list` |
| `cli_plugin_subcommand_unknown(lang, sub)` | `dyyl: unknown subcommand '{sub}'` | `dyyl: 未知子命令 '{sub}'` |

**`--debug` 诊断**：插件调用失败时若 `--debug` 开启，stderr 输出格式与现有 `debug_diagnostic` 一致（line / command / reason 三段），reason 经 i18n。

### 12.3 测试覆盖（i18n 专项）

在 `tests/i18n_tests.rs` 现有基础上扩展：

- 上述每个新增 `pub fn` 在 `Lang::En` 与 `Lang::Zh` 下都返回非空字符串
- 关键函数在两语言下返回**不同**字符串（防止误返回同一字面量）
- 插件 manifest 404 时，`--lang zh` 下 stderr 是中文 `l.dyyapp.com 上未找到插件 ...`
- SHA256 不符时，`--lang en` 下是英文 `SHA256 checksum mismatch`
- `dyyl --lang zh list` 表头是 `名称 版本 上次使用 安装时间`
- `dyyl --lang en list` 表头是 `NAME VERSION LAST_USED INSTALLED`
- 插件返回的 error.message 不被翻译（en/zh 下都显示插件原文）

### 12.4 插件作者的 i18n 提示

插件若要支持双语，可：

1. 在 `plugin_init` 时根据 dyyl 传来的环境（v1 不传 lang；后续 ABI 扩展可加 `init(api_version, lang)` 重载）选择语言
2. 或在自己的 `handle_command` 里维护消息表，按命令参数或配置切换
3. v1 简化：插件消息就一种语言（作者自选），dyyl 透传

文档须明示此约定，避免插件作者误以为 dyyl 会代为翻译。
...
- 插件是否需要知道当前 `lang`？v1 ABI 的 `plugin_init(api_version)` 不传 lang，插件无法做双语；后续可加 `init(api_version, lang)` 或环境变量。文档须明示此限制。
- `dyyl list` 是否输出彩色（带 ANSI）？v1 纯文本，后续可加 `--color` 选项。
- i18n 函数数量较多（约 30 个新增），是否抽一个 `PluginI18n` 子模块组织？实现时按现有 `i18n.rs` 单文件风格加在同一文件，避免过度拆分。
